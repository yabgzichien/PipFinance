 ...
 * issuer signatures, with the lender assessment block) and prints it as a paste-ready
 * SAMPLE_CODE line for LenderConsole/app/tokens.ts, which is its only live consumer.
 *
...
// ── 6. Emit the code for LenderConsole/app/tokens.ts's SAMPLE_CODE ─────────────
// The borrower app has no sample-passport consumer of its own (the in-app lender mirror
// screen was removed), so this no longer writes a source file. Its only live consumer is
// the console's "Load sample" button — paste the line below over `SAMPLE_CODE` there.
const passportCode = JSON.stringify({ passport, signature, issuerSignature });
console.log('\nPaste over SAMPLE_CODE in LenderConsole/app/tokens.ts:\n');
console.log(`export const SAMPLE_CODE =\n  ${JSON.stringify(passportCode)};`);
